...
void move(double xcurrent, double ycurrent, double xback, double yback, double xdesired, double ydesired);
...
void move(double xcurrent, double ycurrent, double xback, double yback, double xdesired, double ydesired) {
	...
	double anglerequired = atan2(ydesired - ycurrent, xdesired - xcurrent) * 180 / PI;
	if (anglerequired < 0) anglerequired += 360;
	...
	if (abs(anglediff) <= 90 || abs(anglediff) >= 270) { //going forward should be faster in this case
		if (anglediff >= 0) {
			pwr = 2000;
			pwl = 1500 + (abs(anglediff) * 500.0 / 90.0); //Scales rotation based on angle change required.
			...
			pwr = 2000;
			pwl = 1500 + (abs(anglediff) * 500 / 270);
		...
		if (180 - abs(anglediff) <= 0) {
			pwr = 1000;
			pwl = 1000 + (abs(anglediff)-180) * 500 / 90;
			...
			pwl = 1000;
			pwr = 1000 + (180 - abs(anglediff)) * 500 / 90;
